...
export async function POST(request: NextRequest) {
  try {
    ...
    // STEP 2: Download and upload to 0G storage
    console.log('Step 2: Simulating 0G storage upload...')
    let storageResult
    try {
    ...
    if (!storageResult.success) {
      ...
      return NextResponse.json(
        { error: 'Failed to upload to 0G storage: ' + storageResult.error },
        { status: 500 }
      )
    }
    ...
    const updatedContract = {
      ...
      stageHistory: [
        ...(contract.stageHistory || []),
        {
          ...
          note: 'Deliverable submitted - GitHub verified, uploaded to 0G storage',
        }
      ]
    }
